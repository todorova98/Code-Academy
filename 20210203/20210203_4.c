...
int binarySearch(int a[], int n, int x){
    int i = n/2; /*достъпваме средния елемент*/
    if(x>a[i]){  /* ако търсеният е по-голям от елемента в средата*/
        while(i<=n){
            if (a[i]==x){
                return i;
            }
    ...
    } else if(x<a[i]){ /* ако търсеният е по-малък от елемента в средата */
        while(i>=0){
            if (a[i]==x){
                return i;
            }
    ...
    } else if (x==a[i]){  /* ако търсеният е равен на елемента в средата */
        return i;
    }
    return -1; /* ако не се среща */
}
int main(){
    int a[]={0,2,1,3,4,5,6,10,8}; /* 4 - среден елемент */
    int n=sizeof(a)/sizeof(a[0]);
    int x;
    printf("Enter number: ");
    scanf("%d", &x);
    printf("The number %d is on position %d in the array", x, binarySearch(a,n,x));
